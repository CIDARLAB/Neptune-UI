<template>
  <v-container id="dashboard" fluid tag="section">
    <v-row>
        ...
        >
          ...
          <template v-slot:actions>
            ...
            <span class="caption grey--text font-weight-light"
              >Last Update: {{ formattimestamp(workspace.updated_at) }}</span
            >
          </template>
        </base-material-workspace-chart-card>
      </v-col>
      ...
      <v-col cols="12" sm="3" lg="3" align="center">
        <div class="my-2">
          ...
          <v-dialog v-model="newworkspacedialog" max-width="300px">
            <v-card>
              ...
              <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn color="success" text @click="createworkspace()">
                  Create
                </v-btn>

                <v-btn color="primary" text @click="newworkspacedialog = false">
                  Cancel
                </v-btn>
              </v-card-actions>
            </v-card>
    ...
    <v-row v-if="files.length > 0">
      <v-col cols="12">
        <div class="font-weight-light mt-1" style="font-size: 25px">Files</div>
      </v-col>
      <v-col cols="12" sm="3" lg="3" v-for="(file, i) in files" :key="i">
      ...
      <v-col cols="12" sm="3" lg="3" v-if="files.length > 0" align="center">
        <div class="my-2">
          <v-tooltip top>
            <template v-slot:activator="{ on }">
              <v-btn
          ...
          <v-dialog v-model="newfiledialog" max-width="300px">
            <v-card>
              <v-card-title> Create New File </v-card-title>
              <v-card-text>
                Please add a file extension name at the end of the filename:
                .lfr or .mint
              </v-card-text>
              <v-card-text>
                <v-text-field
              ...
              <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn color="success" text @click="createfile()">
                  Create
                </v-btn>

                <v-btn color="primary" text @click="newfiledialog = false">
                  Cancel
                </v-btn>
              </v-card-actions>
            </v-card>
...
export default {
  ...
  data: () => ({
    ...
    rules: [
      ...
      (value) =>
        (value && (value.includes(".lfr") || value.includes(".mint"))) ||
  ...
  methods: {
          ...
          for (let wid of response.data) {
            ...
            axios
              ...
              .then((response) => {
                ...
                if (
                  this.$store.getters.currentWorkspace != null &&
                  this.$store.getters.currentWorkspace != undefined
                ) {
                  console.log(
                    " current workspace",
                  ...
                  this.selectworkspace(
                    this.$store.getters.currentWorkspace._id
                  );
                }
              })
              .catch((error) => {
    ...
    deleteworkspace(wid) {
      ...
      axios
        .delete("/api/v1/workspace", config)
        .then((response) => {
