  ...
  <v-container id="grid" fluid tag="section">
    <v-row>
      <v-col cols="12" sm="9" class="pt-0">
        <v-btn color="success" v-on:click="savefile"
          ><v-icon small left light>mdi-content-save</v-icon> Save</v-btn
        >
        <v-btn color="info" v-on:click="savefile(), (compiledialog = true)"
          ><v-icon small left light>mdi-play</v-icon> Save and Compile</v-btn
        >
        <v-btn color="error" v-on:click="deletefile"
          ><v-icon small left light>mdi-delete</v-icon> Delete</v-btn
        >
        <v-btn color="secondary" v-on:click="downloadfile"
          ><v-icon small left light>mdi-cloud-download</v-icon> Download</v-btn
        >
        <v-btn color="secondary" v-on:click="uploadfile"
          ><v-icon small left light>mdi-cloud-upload</v-icon> Upload</v-btn
        >
      </v-col>
    </v-row>
          ...
          <v-list-item three-line>
            <v-list-item-content>
              <v-list-item-title class="headline mb-1">
                <span v-text="fileobject.name"></span>
              </v-list-item-title>
              <v-list-item-subtitle
                >Workspace: <span v-text="currentworkspace.name"></span
              ></v-list-item-subtitle>
            </v-list-item-content>
    ...
    <v-dialog v-model="compiledialog" max-width="500px">
      <v-card>
        ...
        <v-card-actions>
          <v-btn color="primary" text @click="compiledialog = false">
            Close
          </v-btn>
...
export default {
  ...
  methods: {
    ...
    compilefile: function (event) {
      ...
      if (
        this.fileobject.name.match(/\.[0-9a-z]+$/i)[0] === ".uf" ||
        this.fileobject.name.match(/\.[0-9a-z]+$/i)[0] === ".mint"
      ) {
        endpoint = "/api/v1/fluigi";
      } else if (
        this.fileobject.name.match(/\.[0-9a-z]+$/i)[0] === ".lfr" ||
        this.fileobject.name.match(/\.[0-9a-z]+$/i)[0] === ".v"
      ) {
    ...
    downloadfile: function (event) {
      ...
      var fileurl = new URL(
        "/api/v1/downloadFile?id=" + this.fileobject.id,
        document.baseURI
      );
